...
|------|------|------|
| 章节全文 | 用户提供 | 单章完整原文 |
| 章节骨架 | Stage 1 产出 | 本章的 type、weight、keyQuestions、outdatedRisks、prerequisites、subsections.skipIf |
...
|--------------|------|------------|------------|------|
| weight=5, core_principle | Two-pass | 全章原文 | 全文 + Pass 1 笔记 | 质量优先，全深度 |
| weight=3-4, 任意 type | Two-pass | 全章原文 | 全文 + Pass 1 笔记 | 标准流程 |
...
**产出**：Markdown 文件（六层结构，保存为 `/Users/qychen/note/book/{书名}/chapters/ch{id}-extract.md`。目录不存在时自动创建）
...
2. 将结果写入 `/Users/qychen/note/book/{书名}/chapters/ch{chapter_id}-extract.md`（目录不存在则自动创建）
