...
| 模式 | 输入内容 | 适用场景 |
|------|---------|---------|
| A（省 token） | 全书目录 + 前言/序言 + 元信息 | 已有提取好的目录文本 |
| B（省人工） | 全书全文 | 直接扔原文，Skill 自行提取目录和前言 |
...
**输出路径**：`/Users/qychen/note/book/{书名}/stage1-skeleton.json`。
- `{书名}` 从输入元信息中提取，去除副标题和特殊字符，如 "深入理解Java虚拟机"
- 目录自动创建，无需用户手动建
...
8. **复杂度热点自动检测** —— 复杂小节（原文长 + 脚注多 + 跨章引用多）需要在 Stage 2 启用深度脚手架，而不是精炼提纯。检测规则使用客观指标，不依赖 LLM 主观判断

### 复杂度热点检测规则

在解析目录和原文时，按以下客观指标计算每个小节的复杂度：

| 指标 | 阈值 | 说明 |
|------|------|------|
| 原文小节长度 | > 3000 字 | 作者用篇幅投票，长小节通常概念层数多 |
| 脚注密度 | > 3 个 | 脚注多 = 不额外解释读者无法理解 |
| 跨章引用次数 | > 2 个 | "见第 X 章"、"参见 §X.X" 出现越多，概念栈越深 |

**判定逻辑**：三项全部命中 → `complexity: "high"`；命中 1-2 项 → `complexity: "medium"`；其余 → `complexity: "low"`。
...
- [ ] `complexity` 判定有客观依据（字数、脚注数、跨章引用数），非主观推测
...
【主任务】
0. 从书名提取目录名（去除副标题和特殊字符），自动创建 `/Users/qychen/note/book/{书名}/` 目录
1. 为每一章标注 type（core_principle/practical/reference/outdated）、weight（1-5）、practiceDensity
2. 为每个 subsection 计算复杂度（complexity: high/medium/low），规则：
   - 原文小节长度 > 3000 字 + 脚注 > 3 + 跨章引用 > 2 → high
   - 命中 1-2 项 → medium
   - 其余 → low
...
【输出要求】
- 严格按 JSON schema 输出
- 将结果写入 `/Users/qychen/note/book/{书名}/stage1-skeleton.json`（目录不存在则自动创建）
- 不要总结章节内容，只做元数据标注
